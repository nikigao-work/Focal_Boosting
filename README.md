...
2. **Фокусирующий модификатор**:
   ...
   - Если пример легко классифицируется ((p_t -> 1)), то ((1 - p_t)^gamma -> 0), и его вклад в градиент будет минимальным.
   - Если пример трудный ((p_t -> 0)), то ((1 - p_t)^gamma -> 1), и он получит больший вес в оптимизации.
...
Где zₖ — логит для класса k, а Σₖ exp(zₖ) — нормировка по всем классам.

Фокальный лосс состоит из двух частей:

(- (1 - pₜ)ᵧ log(pₜ)) — это основной компонент, который штрафует за ошибки.
Производная по pₖ даёт комбинированный штраф, который зависит от параметра γ, (1 - pₜ) и pₜ.
...
1. **Сохраняется физический смысл**:
    - (yₖ - p̂ₖ): Ошибка между истинным распределением и предсказанным.
    - (1 - p̂ₖ)ᵧ⁻¹ * p̂ₖ: Усиливает вклад трудных примеров (когда p̂ₖ мало) и уменьшает влияние лёгких примеров (когда p̂ₖ близко к 1).
...
4. **Простота реализации**:
   - Легко интегрируется в бустинг (как в вашем коде).
