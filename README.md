...
Итоговый градиент для обновления логита $$z_k$$ выглядит так:
...
$$\[
\frac{\partial FL_i}{\partial z_{i,k}} = (y_{i,k} - p_{i,k}) \cdot \gamma \cdot (1 - p_{i,k})^{\gamma - 1} \cdot p_{i,k}
\]$$
...
2. $$(1 - p_{i,k})^{\gamma - 1}$$ — фокусирующий фактор, усиливающий вклад сложных примеров.
...
- `(1 - probs) ** (self.gamma - 1)` реализует фокусирующий фактор $$(1 - p_{i,k})^{\gamma - 1}$$, который увеличивает влияние сложных примеров (когда модель ошибается).
